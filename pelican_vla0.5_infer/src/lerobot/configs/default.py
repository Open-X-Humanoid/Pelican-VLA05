...
def get_safe_default_codec() -> str:
    """Default video decode backend for dataset configs.

    Inlined from LeRobot's ``datasets.video_utils`` so the inference package does
    not depend on the (training-only) video stack.
    """
    return "torchcodec"
...
class DatasetConfig(draccus.ChoiceRegistry, abc.ABC):
    ...
    max_loaded_datasets: int = 0  # LRU cap on loaded Arrow tables (0 = keep all)
    steps_per_epoch: int = 0  # Worker recycle interval; 0 = full epoch (no forced recycle)
